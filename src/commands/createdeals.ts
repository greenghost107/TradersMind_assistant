...
export const data = new SlashCommandBuilder()
  .setName('createdeals')
  .setDescription('Create interactive buttons for stock symbols from your recent message (Deals channel only)');
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  // Check if deals channel is configured
  if (!config.dealsChannel) {
    await interaction.reply({ 
      content: '❌ Deals channel not configured - please contact administrator', 
      ephemeral: true 
    });
    return;
  }

  // Validate this command is used in the deals channel only
  if (interaction.channel?.id !== config.dealsChannel) {
    await interaction.reply({ 
      content: '❌ This command only works in the deals channel', 
      ephemeral: true 
    });
    return;
  }
  ...
  setTimeout(async () => {
    ...
    Logger.error('Error in /createdeals command:', error);
    
    try {
      await interaction.editReply({ 
        content: '❌ An error occurred while creating deals buttons' 
      });
    } catch (editError) {
